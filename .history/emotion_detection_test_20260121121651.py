...
def process_journal(text):
    ...
    return {
        "Primary Emotion": final_emotion,
        "Top-3 Emotions": get_top_k_emotions(sorted_emotions, k=3)
    }
...
def evaluate_model(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        y_pred.append(prediction["Primary Emotion"])
...
def user_input_mode():
    ...
    while True:
        ...
        result = process_journal(text)

        print("\nPredicted Emotion:", result["Primary Emotion"])
        print("Top-3 Emotions   :", result["Top-3 Emotions"])
        print("-" * 40)
